...
const dot=document.getElementById('cursorDot'),ring=document.getElementById('cursorRing');let mx=0,my=0,rx=0,ry=0;addEventListener('mousemove',e=>{mx=e.clientX;my=e.clientY;dot.style.cssText+=`;opacity:1;left:${mx}px;top:${my}px`;ring.style.opacity=1});(function loop(){rx+=(mx-rx)*.16;ry+=(my-ry)*.16;ring.style.left=rx+'px';ring.style.top=ry+'px';requestAnimationFrame(loop)})();document.querySelectorAll('a,button,.project-card').forEach(x=>{x.onmouseenter=()=>ring.classList.add('hover');x.onmouseleave=()=>ring.classList.remove('hover')});
document.querySelectorAll('.project-card').forEach(c=>{c.classList.add('tilt-card');c.onmousemove=e=>{const r=c.getBoundingClientRect(),x=(e.clientX-r.left)/r.width-.5,y=(e.clientY-r.top)/r.height-.5;c.style.transform=`perspective(900px) rotateX(${y*-7}deg) rotateY(${x*9}deg) scale(1.015)`};c.onmouseleave=()=>c.style.transform=''});
